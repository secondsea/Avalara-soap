...
//die();	
	...
	// retrieves item records from the avatax export stored procedure.
	public function getRMAnumber ($conitemno, $taxBiz) {
//$sp = 'sandbox.dbo.avatax_export'; 
echo $taxBiz ." \n ";
echo $conitemno . "\n";

//'SELECT [RAID] from [dash_albert].[DBO].[rmaitem] WHERE [CONITEMNO] = 11643655 and  exists (select itemno FROM [dash_albert].[dbo].[AVATAX] where  itemno =[RAID])
	$sql="SELECT [RAID]  from [" . $taxBiz ."].[DBO].[rmaitem] WHERE [CONITEMNO] = '" . $conitemno . "' and exists (select itemno FROM [".$taxBiz."].[dbo].[AVATAX] where  itemno =[RAID])" ;
	//echo $sql . "\n";
	
	//status	FROM [SANDBOX].[DBO].[AvalaraResults] WHERE [ORDERNO] = '" . $contractno . "'";
...
// Update tax item records - bulk update of all item records of the tax request.
	public function updateTaxItemRecords($taxRecord) {
		$sql="UPDATE [SANDBOX].[dbo].[AvalaraResults] set [TAXAMOUNT] = '". $taxRecord['taxamount'] ."',[MESSAGE]= '". $taxRecord['message'] . "'   WHERE [ORDERNO] = '" . $taxRecord['orderno']. "'";		 
//var_dump($sql);		
		try{
			$pdo_query = $this->resq->prepare($sql);
			$pdo_query->execute();
	...
	//	$sql="SELECT COUNT(*) AS [RECORDCOUNT], (Select top 1 status from [SANDBOX].[DBO].[AvalaraResults] WHERE [ORDERNO] = '" . $contractno . "' )status	FROM [SANDBOX].[DBO].[AvalaraResults] WHERE [ORDERNO] = '" . $contractno . "'";
		//var_dump($sql);
	  //  $sql="SELECT COUNT(*) AS [RECORDCOUNT]	FROM [SANDBOX].[DBO].[AvalaraResults] WHERE [ORDERNO] = '" . $contractno . "'";
		...
		//	var_dump($result);
		//$rowCount= (int)$result['RECORDCOUNT'];
	   //	var_dump ($rowCount);
		//	}	
