...
//		$is_test = stripos($project_path, '/web/') !== false; // sfContext::getInstance()->getUser()->is_test();
	...
	public function processTax()
	{
        ...
        // $num = 50;
         ...
         while( $recOffset < $recCountSI) {
          $taxContracts=$this->resq->getTaxContractsSI($recOffset);  // use a different fetch that uses limit 100 or so
		...
		$recoffset=0
 		while ($offset < $recCountMU) {
			$taxContracts=$this->resq->getTaxContracts($recOffset);   
			
			
			$this->procesContractblockMU(taxContracts)
	...
	public function procesContractblockSI($taxContracts)
	{
		foreach ($taxContracts as $this->contractInfo)  {
		echo "**********Record ITEM NUMBER: " . $this->taxContract["ITEMNO"] ."*************************************" , PHP_EOL;
		...
		public function setSingleLineArray () {
			...
			$OneLinz = new Line();
			$OneLine->setNo($LN);
			$OneLine->setItemCode($contractInfo["MASTERNO"]);
			$OneLine->setDescription($contractInfo["DESCRIPTION"]);
			$OneLine->setQty($contractInfo["REVQTY"]);
			$OneLine->setAmount($contractInfo["PRICEPER"]);
		...
		try {
			...
			$this->taxRequest->setDocCode($this->contractInfo["ITEMNO"]); // contract no
	...
	public function procesContractblockMU(taxContracts)
	{
...
//		}
	...
	//	} 
		...
		$this->taxRequest->setDocCode($this->contractInfo["CUSTOMERNO"]); // contract no
		//$this->postTaxRequest->setDocCode($this->taxNo);
